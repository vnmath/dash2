...
max_val = df.max().max()#max of each feature, then max of them
##App layout, about HTML
app.layout = html.Div([# Big division
    html.Div([
        ...
        html.Div([#Left Div
            ...
            dcc.Dropdown(
                ...
                options=[
                    {'label': 'Principal Component Analysis', 'value': 'PCA'},#show label, execute value
                ...
                value='PCA', #default value to show to custumer
                clearable=False #donot execute all at the same time

            )], style={'width': '49%', 'display': 'inline-block'}
        ),

        html.Div([#Rigth Div float right
...
@app.callback(#call back if there is any parametre change
...
def update_graph(feature, model, gradient):
    ...
    if feature == 'None':#show plot of raw data
        cols = None #define color
        sizes = None # same size of dots
        hover_names = [f'Customer {ix:03d}' for ix in df.index.values]
    elif feature in ['Region', 'Channel']:#categorical data
